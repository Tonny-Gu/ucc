...
#ifndef ALLGATHER_OFFLOAD_DPU_H_
#define ALLGATHER_OFFLOAD_DPU_H_

#include <ucs/debug/log_def.h>
#include "dpu_offload_service_daemon.h"
#include "allgather_offload.h"
...
static size_t
unpack_allgather_offload_args(void *data, allgather_offload_args_t *args)
{
    int i;
    size_t total_size = 0;
...
static int
ag_import_memh(execution_context_t *context, void *address, size_t length,
...
static int create_allgather_offload_subops(allgather_offload_coll_t *op)
{
    ...
    for (i = 1; i < size; i++) {
        ...
        allgather_offload_subop_t *subop =
            malloc(sizeof(allgather_offload_subop_t));
        if (!subop) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        subop->type   = ALLGATHER_OFFLOAD_SEND;
        subop->peer   = peer;
    ...
    for (i = 1; i < size; i++) {
        ...
        op->r_todo++;
        subop->status = UCS_INPROGRESS;
        subop->type   = ALLGATHER_OFFLOAD_RECV;
        subop->peer   = peer;
...
/* calculate the size of allgather_offload_dpu_rts_t data structure */
static size_t get_allgather_offload_dpu_rts_packed_size(size_t rkey_len)
{
    size_t total_size = 0;

    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, coll_type);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, tag);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, group_id);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, rank);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, peer);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, padding);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, start);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, length);
    total_size += FIELD_SIZEOF(allgather_offload_dpu_rts_t, rkey_len);
    total_size += rkey_len;
...
static size_t
pack_allgather_offload_dpu_rts(allgather_offload_coll_t *op,
...
static int
ag_add_receive(execution_context_t *context, allgather_offload_coll_t *op,
            allgather_offload_subop_t *subop,
            allgather_offload_dpu_rts_t *args)
{
    ...
    allgather_offload_rdma_read_t *item =
        malloc(sizeof(allgather_offload_rdma_read_t));
    if (!item) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    memcpy(item->s_rkey_buf, (void *)args +
           FIELD_OFFSET(allgather_offload_dpu_rts_t, rkey_buf),
           args->rkey_len);
...
static int
ag_complete_op(execution_context_t *context, allgather_offload_coll_t *op)
{
    int rc;
    ...
    size_t packed_size = sizeof(allgather_offload_dpu_done_t);
    ...
    allgather_offload_dpu_done_t *args_buf = event->payload;
    ...
    dest_client_t c = GET_CLIENT_BY_RANK(host_context, op->args.group_id,
                                         op->args.rank);
    assert(c.ep);
    rc = event_channel_emit(&event, UCC_TL_UCP_ALLGATHER_DPU_DONE_AM_ID,
                            c.ep, c.id, NULL);
...
static int
ag_host_arrive_am_cb(struct dpu_offload_ev_sys *ev_sys,
                  ...
                  size_t hdr_size, void *data, size_t data_len)
{
    ...
    allgather_offload_coll_t *op = malloc(sizeof(allgather_offload_coll_t));
    if (!op) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    allgather_offload_args_t *args = &op->args;
    size_t unpacked_size = unpack_allgather_offload_args(data, args);
    ...
    rc = ag_import_memh(context, (void *)args->s_start, args->s_length,
                     args->s_rkey_buf, &op->s_memh);
    if (rc) {
        return UCS_ERR_NO_MEMORY;
    }
    rc = ag_import_memh(context, (void *)args->r_start, args->r_length,
    ...
    /* prepare subops */
    rc = create_allgather_offload_subops(op);
    if (rc) {
        ucs_error("create_allgather_offload_subops() failed");
        return rc;
    }
    ...
    allgather_offload_subop_t *subop = NULL, *subop_item, *subop_tmp;
    ucs_list_for_each_safe(subop_item, subop_tmp, &op->s_pending, super) {
        ...
        /* get an event from event pool, allocate event buffer */
        size_t packed_size = get_allgather_offload_dpu_rts_packed_size(
                                op->s_rkey_len);
        dpu_offload_event_info_t event_info = { .payload_size = packed_size, };
        dpu_offload_event_t *event;
        ...
        size_t packed_size_tmp = pack_allgather_offload_dpu_rts(op,
                                    subop->peer, event->payload);
        assert(packed_size == packed_size_tmp);

        /* send RTS notification to remote DPU about data readiness */
        rc = event_channel_emit(&event, UCC_TL_UCP_ALLGATHER_DPU_RTS_AM_ID,
...
static int
ag_dpu_rts_am_cb(struct dpu_offload_ev_sys *ev_sys, execution_context_t *context,
              ...
              size_t data_len)
{
    ...
    allgather_offload_dpu_rts_t *args = (allgather_offload_dpu_rts_t *)data;
    int rc;

    /* find receiver from active_colls */
    allgather_offload_coll_t *op = NULL, *op_item, *op_tmp;
    ...
    if (op == NULL) {
        ...
        allgather_offload_dpu_rts_event_t *item =
            malloc(sizeof(allgather_offload_dpu_rts_event_t));
        if (!item) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        allgather_offload_dpu_rts_t *payload = malloc(data_len);
    ...
    rc = ag_add_receive(context, op, subop, args);
    if (rc) {
        ucs_error("ag_add_receive() failed");
        return UCS_ERR_NO_MESSAGE;
    }
...
static int
ag_dpu_ack_am_cb(struct dpu_offload_ev_sys *ev_sys, execution_context_t *context,
              struct am_header *hdr, size_t hdr_size, void *data,
              size_t data_len)
{
    ucs_debug("received %zu bytes from client %zu for message type %zu context %p",
    ...
    allgather_offload_dpu_ack_t *args = (allgather_offload_dpu_ack_t *)data;
    int rc;
...
static int register_allgather_dpu_notifications(offloading_engine_t *engine)
{
    int rc;

    rc = engine_register_default_notification_handler(engine,
            UCC_TL_UCP_ALLGATHER_HOST_ARRIVE_AM_ID,
            ag_host_arrive_am_cb);
    if (rc) {
...
static int ag_progress_rts(execution_context_t *context)
{
    int rc;

    /* check pending RTS, if found creat a new receive op */
    allgather_offload_dpu_rts_event_t *rts = NULL, *rts_item, *rts_tmp;
    allgather_offload_coll_t *op = NULL, *op_item, *op_tmp;
    allgather_offload_subop_t *subop = NULL, *subop_item, *subop_tmp;
            ...
            if (rts_item->payload->coll_type == op_item->args.coll_type &&
                ...
                ucs_list_for_each_safe(subop_item, subop_tmp, &op->r_pending,
                    ...
                    if (rts_item->payload->rank == subop_item->peer) {
                        ...
                        /* add this receive op to pending_receives list */
                        rc = ag_add_receive(context, op, subop, rts->payload);
                        if (rc) {
                            ucs_error("ag_add_receive() failed");
                            return UCS_ERR_NO_MESSAGE;
                        }
...
/* progress receives */
static int ag_progress_receive(execution_context_t *context)
{
    int rc;

    /* find completed receive ops */
    allgather_offload_rdma_read_t *recv = NULL, *recv_item, *recv_tmp;
    allgather_offload_coll_t *op;
    allgather_offload_subop_t *subop;
    ucs_list_for_each_safe(recv_item, recv_tmp, &posted_receives, super) {
        ...
        if (status == UCS_OK) {
            ...
            /* get an event from event pool, allocate event buffer */
            size_t packed_size = sizeof(allgather_offload_dpu_ack_t);
            ...
            allgather_offload_dpu_ack_t *args_buf =
                (allgather_offload_dpu_ack_t *)sp_event->payload;
            args_buf->coll_type = op->args.coll_type;
            args_buf->tag       = op->args.tag;
            args_buf->group_id  = op->args.group_id;
            args_buf->rank      = op->args.rank;
    ...
    for (int i = 0; (i < max_receives - posted_count) &&
        ...
        allgather_offload_rdma_read_t *item =
            ucs_list_extract_head(&pending_receives,
                                  allgather_offload_rdma_read_t, super);
...
static int ag_progress_ops(execution_context_t *context)
{
    int rc;
    rc = ag_progress_rts(context);
    rc = ag_progress_receive(context);

    return UCS_OK;
}
#endif //ALLGATHER_OFFLOAD_DPU_H_
