...
ucc_base_coll_alg_info_t
    ucc_tl_ucp_allgather_algs[UCC_TL_UCP_ALLGATHER_ALG_LAST + 1] = {
        ...
        [UCC_TL_UCP_ALLGATHER_ALG_OFFLOAD] =
            {.id   = UCC_TL_UCP_ALLGATHER_ALG_OFFLOAD,
             .name = "offload",
                             ...
                             size_t r_rkey_buf_size)
{
    ...
    total_size += FIELD_SIZEOF(allgather_offload_args_t, tag);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, group_id);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, size);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, rank);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, padding);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, s_start);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, s_length);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, r_start);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, r_length);
    total_size += comm_size *
        FIELD_SIZEOF(allgather_offload_args_t, r_displacements[0]);
    total_size += comm_size *
        FIELD_SIZEOF(allgather_offload_args_t, r_counts[0]);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, s_rkey_len);
    total_size += FIELD_SIZEOF(allgather_offload_args_t, r_rkey_len);
...
size_t
pack_allgather_offload_args(ucc_tl_ucp_task_t *task, void *s_start,
...
ucc_status_t ucc_tl_ucp_allgather_offload_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t      *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_tl_ucp_team_t      *team = TASK_TEAM(task);
    ucc_tl_ucp_lib_t       *lib  = TASK_LIB(task);
    allgather_host_coll_t *op   = NULL, *op_item, *op_tmp;
...
ucc_status_t ucc_tl_ucp_allgather_offload_start(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t      *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_tl_ucp_team_t      *team = TASK_TEAM(task);
    ucc_tl_ucp_lib_t       *lib  = TASK_LIB(task);
    ...
    size_t offload_args_packed_size =
        pack_allgather_offload_args(task, s_start, s_len, r_start, r_len,
    ...
    status = ucc_tl_ucp_allgather_offload_progress(coll_task);
                                                ...
                                                ucc_coll_task_t     **task_h)
{
    ...
    *task_h = &task->super;
    ...
    /* initialize active_colls if this is the first allgather coll */
    ...
    allgather_host_coll_t *op = malloc(sizeof(allgather_host_coll_t));
    if (!op) {
    ...
    task->super.post     = ucc_tl_ucp_allgather_offload_start;
    task->super.progress = ucc_tl_ucp_allgather_offload_progress;
    return UCC_OK;
}
