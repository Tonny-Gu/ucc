...
#ifndef ALLGATHER_OFFLOAD_H_
#define ALLGATHER_OFFLOAD_H_
...
enum allgather_ext_am_id {
    ...
    /* Last AM message ID */
    UCC_TL_UCP_ALLGATHER_LAST_AM_ID
};

enum allgather_offload_subop_type {
    ALLGATHER_OFFLOAD_SEND = 0,
    ALLGATHER_OFFLOAD_RECV = 1
};
 ...
 * information for the DPU to start the allgather operation.
 */
typedef struct allgather_offload_args {
 ...
 */
typedef struct allgather_offload_coll {
    ucs_list_link_t               super;        /* op list item */
    ucs_status_t                  status;       /* op status */
    allgather_offload_args_t      args;         /* coll args for this op */
...
} allgather_offload_subop_t;

/* DPU RTS notification bookkeeping list item. */
typedef struct allgather_offload_dpu_rts_event {
    ucs_list_link_t                  super;      /* notification list item */
    allgather_offload_dpu_rts_t     *payload;    /* payload buffer */
} allgather_offload_dpu_rts_event_t;

/* RDMA READ bookkeeping list item. */
typedef struct allgather_offload_rdma_read {
    ucs_list_link_t              super;      /* op list item */
    ucs_status_t                 status;     /* op status */
    ...
    allgather_offload_coll_t    *op;         /* original receive op */
    allgather_offload_subop_t   *subop;      /* original receive subop */
} allgather_offload_rdma_read_t;
#endif //ALLGATHER_OFFLOAD_H_
